# Beacon Relay — Technical Build Specification for Kimi

**Purpose of this document:** a complete engineering brief for building Beacon Relay — the remote hospital infrastructure monitoring appliance for small, rural, and critical-access hospitals. This answers "what does it take to build this" concretely: not one program, but three real subsystems that have to work together — a device image/configurator pipeline, the device-side monitoring agent, and a cloud control plane with a fleet dashboard. Each is scoped below with real, purpose-built tools rather than from-scratch reinvention wherever a solved tool already fits.
...
```
HOSPITAL SITE (mini PC, on their network)              │  BEACON_RELAY CONTROL PLANE (cloud)
                                                          │
  ┌────────────────────────┐   outbound-only, mTLS       │
  │      DEVICE AGENT       │──────────────────────────▶ │   ┌──────────────────┐
  │  ┌──────────────────┐  │                              │   │  Ingestion API    │
...
This is a tool that runs on Ryan's machine, not on the appliance. Its job: produce one signed, versioned, bootable image per device, and a first-boot provisioning flow that turns a blank mini PC into a trusted, enrolled Beacon Relay appliance.
...
{
  "event_id": "uuid-v4",
  "device_id": "beacon-relay-device-uuid",
...
**Storage:** PostgreSQL for device registry, config, RBAC, audit log, and incident records — this doesn't need Handoff's ClickHouse-scale trace storage; Beacon Relay's telemetry volume (periodic health checks per site, not per-message tracing across a busy interface engine) is orders of magnitude lower. Add the TimescaleDB extension to the same Postgres instance if the time-series query patterns need it later — don't stand up a second database system before there's a real reason to.
...
Own the logic (severity tiers, owner mapping, plain-language impact statements, runbook attachment, acknowledgment tracking, suppression/maintenance windows) as real product IP — but don't rebuild message delivery from scratch. Use Twilio for SMS/voice and SES or SendGrid for email, with a Teams/Slack webhook for chat — proven delivery infrastructure, with the escalation *decisions* (who, when, how loud, what's attached) as the actual thing Beacon Relay is selling.
...
**React frontend**, paired with the same Node/TS API layer — reuse patterns from Handoff's console rather than starting the visual design over, though Beacon Relay's own status semantics (Reachable/Verified Ready/Active, not Nominal/Watch/Critical) should read as their own thing, not a reskin.
...
| Registry/config/audit/incidents | PostgreSQL (+TimescaleDB if needed) | Beacon Relay's telemetry volume doesn't justify a separate time-series store yet |
...
```
You are building "Beacon Relay," a remote hospital infrastructure monitoring appliance for
