# Beacon Relay — Cloud Architecture on AWS (Control Plane Hosting)

This is a companion document to `docs/specs/BEACON_RELAY_BUILD_SPEC.md`, `docs/specs/BEACON_RELAY_EHR_INTEGRATIONS.md`, and
`docs/specs/BEACON_RELAY_CONTROLS_AND_IDENTITY.md`. It does not replace any of them — the event schema,
...
to host the Beacon Relay Control Plane (`docs/specs/BEACON_RELAY_BUILD_SPEC.md` Section 5–7) heavily on AWS, what
...
| Beacon Relay component (spec section) | AWS service | Why this one |
|---|---|---|
| Device registry + ingestion, outbound-only mTLS (`docs/specs/BEACON_RELAY_BUILD_SPEC.md` §5) | **AWS IoT Core**, with a customer-managed CA registration pointing at step-ca | Purpose-built for exactly this device model — per-device mTLS cert, outbound-only connection, device shadow/state. step-ca stays the actual issuing CA; IoT Core just trusts it. |
| Signed OTA rollout, staged (`docs/specs/BEACON_RELAY_BUILD_SPEC.md` §2, §3, §8 step 7) | **AWS IoT Jobs**, orchestrating the RAUC bundle already produced by the Configurator | Per-device job status (queued/in-progress/succeeded/failed/rejected), staged rollout groups, built-in retry — the distribution layer over an artifact whose *format* stays RAUC, unchanged. |
| Remote-support session broker / Action Registry transport (`docs/specs/BEACON_RELAY_BUILD_SPEC.md` §7, `docs/specs/BEACON_RELAY_CONTROLS_AND_IDENTITY.md` §2) | **AWS IoT Secure Tunneling** | Already implements "device behind NAT, no inbound port, opens an outbound tunnel on a time-limited scoped token" — the literal mechanism Section 7 describes, as a managed service. |
| Control-plane API, ingestion processor, console backend (`docs/specs/BEACON_RELAY_BUILD_SPEC.md` §5, §7) | **ECS on Fargate**, behind an ALB | Proportionate to Beacon Relay's actual telemetry volume (periodic per-site checks, not high-frequency tracing) — EKS would solve a scale problem this product doesn't have. |
| Alerting & Escalation Engine logic (`docs/specs/BEACON_RELAY_BUILD_SPEC.md` §6) | **Step Functions**, calling Twilio/SES/SendGrid as already decided | The escalation logic (wait for ack, timeout, escalate to next contact) is a wait-then-branch state machine; Step Functions gives a visual, auditable execution history of every escalation for free. |
| Action Registry session issuance (`docs/specs/BEACON_RELAY_CONTROLS_AND_IDENTITY.md` §2) | **Lambda + DynamoDB** (TTL on session-token items) | Natural fit for short-lived, scoped tokens; DynamoDB TTL expires a session automatically without a cleanup job. |
| Device registry, config, RBAC, incident records (`docs/specs/BEACON_RELAY_BUILD_SPEC.md` §5) | **RDS for PostgreSQL, Multi-AZ** | As already decided — Beacon Relay's telemetry volume doesn't justify a separate time-series store yet. Confirm TimescaleDB extension availability on the target RDS version before relying on it; native monthly partitioning is the fallback if it isn't supported. |
| Audit log — append-only, tamper-evident (`docs/specs/BEACON_RELAY_BUILD_SPEC.md` §5) | **Amazon QLDB**, or an S3-Object-Lock-backed export of the Postgres audit table | This is a step beyond "just don't issue UPDATE grants" — a cryptographically verifiable ledger is a genuinely stronger claim to make in a hospital security review, and mirrors the same tamper-evidence property already designed into the backup-risk Tier 2 check. |
| Console auth, RBAC roles (`docs/specs/BEACON_RELAY_BUILD_SPEC.md` §5) | **Amazon Cognito** (user pools, one group per role) | Five roles (support technician, customer IT admin, operations manager, security auditor, read-only executive) map directly to Cognito groups; MFA enforced above read-only; SAML/SSO federation available later without an auth rewrite. |
| Fleet Console frontend (`docs/specs/BEACON_RELAY_BUILD_SPEC.md` §7) | **React on S3 + CloudFront**, behind WAF | Static hosting for the SPA; WAF in front of both this and the API Gateway/ALB. |
...
available: three separate pieces of custom infrastructure implied by `docs/specs/BEACON_RELAY_BUILD_SPEC.md` — the
...
as `docs/specs/BEACON_RELAY_BUILD_SPEC.md` §2 and §8 Phase 1 already specify. IoT Core is registered to trust that CA,
not the other way around — Beacon Relay doesn't hand root trust to AWS.

At Beacon Relay's realistic scale (low-thousands of devices, periodic heartbeats measured in
...
already appears at the end of every phase in `docs/specs/BEACON_RELAY_BUILD_SPEC.md` §8 and every Kimi prompt in this
spec set — instead of being an ad hoc manual step each time, it's a standing environment.

**Backup/DR ("backup"), for Beacon Relay's own control-plane resilience.** This is a different
thing from the device-side backup-risk feature in `docs/specs/BEACON_RELAY_CONTROLS_AND_IDENTITY.md` §2 —
that feature checks a *hospital's* backup posture; this is about Beacon Relay's own. Multi-AZ RDS,
...
notice. This matters more than it might for a typical SaaS product: Beacon Relay's core promise is
...
anything about the `phi_mode` design in `docs/specs/BEACON_RELAY_BUILD_SPEC.md` §4; it's a hosting-layer prerequisite
...
1. **AWS Organization + three accounts** (prod, sim/staging, security/log-archive) — this
   is foundational the same way step-ca/schema was foundational in `docs/specs/BEACON_RELAY_BUILD_SPEC.md` §8 Phase 1;
...
2. **IoT Core + step-ca CA registration**, tested against Phase 1's existing enrollment
   flow (`docs/specs/BEACON_RELAY_BUILD_SPEC.md` §8 Phase 1) — confirm a device enrolling via step-ca is correctly
...
5. **IoT Jobs wired to the Configurator's RAUC bundle output** (`docs/specs/BEACON_RELAY_BUILD_SPEC.md` §2, §12) —
...
6. **IoT Secure Tunneling + the Action Registry's Lambda/DynamoDB session layer**
   (`docs/specs/BEACON_RELAY_CONTROLS_AND_IDENTITY.md` §2, §4) — build after step 4, since it depends on the
...
```
You are continuing work on "Beacon Relay." Read docs/specs/BEACON_RELAY_CLOUD_ARCHITECTURE_AWS.md in full before
starting. This document specifies how the control plane (docs/specs/BEACON_RELAY_BUILD_SPEC.md Section 5–7)
...
2. Stand up AWS IoT Core in the prod account, registered to trust step-ca (already built
   per docs/specs/BEACON_RELAY_BUILD_SPEC.md Phase 1) as a customer-managed CA. Test against the existing device
...
3. Stand up RDS for PostgreSQL (Multi-AZ) and Amazon Cognito with five user-pool groups
   matching the RBAC roles in docs/specs/BEACON_RELAY_BUILD_SPEC.md Section 5 (support technician, customer IT admin,
