# Beacon Relay — Fleet Topology Views & Troubleshooting Memory
...
3. **Ticketing system export** — not a ticketing system of Beacon Relay's own, but a way to get a
   well-formed incident record into whatever ticketing system (or plain inbox) a hospital
   already uses, so Beacon Relay never asks IT staff to adopt a second system.
...
Beacon Relay does not build a ticketing/ITSM system, full stop. Every target hospital already has
...
write to a system outside Beacon Relay's own control plane, so it gets the same treatment as any
