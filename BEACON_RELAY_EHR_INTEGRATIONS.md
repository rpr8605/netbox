...
hospital systems Beacon Relay needs to recognize and check.
...
```
You are continuing work on "Beacon Relay," the remote hospital infrastructure monitoring
...
**Beacon Relay does not build fifteen deep, vendor-certified EHR integrations.** That would mean
...
Instead, Beacon Relay builds a **small number of protocol-level adapters**, plus a **per-site
...
## 3. What Beacon Relay watches, mapped to L0–L4
...
Most target hospitals don't hand Beacon Relay a raw feed from the EHR itself — they run a
...
- **Mirth Connect / NextGen Connect** — free, open-source, and by a wide margin the most
  ...
  Beacon Relay needing to see message contents — this is a legitimate, low-risk read that adds
...
| # | System | Current owner / brand | What it is | Why it's in scope here | Integration available | Realistic Beacon Relay tier |
|---|---|---|---|---|---|---|
...
What "monitoring Surescripts connectivity" concretely means for Beacon Relay, given the
passive/read-only design constraint: Beacon Relay is never going to be the thing that
originates or intercepts a real prescription transaction — that stays entirely inside the
EHR's own certified e-prescribing module. What Beacon Relay *can* legitimately check:
...
- Do **not** build a native Surescripts network integration (it requires Surescripts'
  ...
  monitoring appliance's). Beacon Relay watches the connection from the outside; it doesn't
  join the network.
...
scoring vendors partly on USCDI v3 readiness. This is directly useful for Beacon Relay: it means
...
not an afterthought bolted onto the HL7 work — by the time Beacon Relay is shipping devices in
...
- No write-back to any EHR, interface engine, or pharmacy network. Every integration in
  this document is read-only or passive-tap-only. Beacon Relay never originates a clinical
...
This section exists because of a real question: should Beacon Relay pull in Handoff (the
...
**yes, but scoped narrowly** — topology visibility only, built on data Beacon Relay is already
...
Handoff's job. Beacon Relay's job here is just: show what's talking to what, and whether it's
...
Beacon Relay-specific vocabulary for the same concept. Nobody is asking Kimi to integrate the two
codebases right now — but if a hospital later runs both Beacon Relay and full Handoff, or if a
...
what the admin-API reader already exposes per channel. That's the line between "Beacon Relay
shows you interfaces" and "Handoff explains and prices what they're costing you" — the
second one stays a separate, deeper product, not a Beacon Relay feature, per the scope decision
made for this build.
