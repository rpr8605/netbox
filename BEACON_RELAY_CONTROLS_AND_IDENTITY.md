...
thing that actually makes Beacon Relay defensible — deep healthcare-IT and EHR knowledge, not
...
and core network gear). This protects the IT stack Beacon Relay already depends on rather than
...
| TLS certificate expiration | Periodic cert inspection (expiry date, issuer, chain validity) on every endpoint Beacon Relay already checks — this is nearly free, since the TLS handshake step (L2) already exists | None planned |
...
(an outside vendor providing account support into a customer's tenant): don't have Beacon Relay,
...
similar), sold as an add-on to Beacon Relay, not a feature of the appliance's software. This is
