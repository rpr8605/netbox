...
This is a companion document to `BEACON_RELAY_BUILD_SPEC.md`, `BEACON_RELAY_EHR_INTEGRATIONS.md`, and
`BEACON_RELAY_CONTROLS_AND_IDENTITY.md`. It does not replace any of them — the event schema,
...
to host the Beacon Relay Control Plane (`BEACON_RELAY_BUILD_SPEC.md` Section 5–7) heavily on AWS, what
...
| Control-plane API, ingestion processor, console backend (`BUILD_SPEC` §5, §7) | **ECS on Fargate**, behind an ALB | Proportionate to Beacon Relay's actual telemetry volume (periodic per-site checks, not high-frequency tracing) — EKS would solve a scale problem this product doesn't have. |
...
thing from the device-side backup-risk feature in `BEACON_RELAY_CONTROLS_AND_IDENTITY.md` §2 —
...
```
You are continuing work on "Beacon Relay." Read BEACON_RELAY_CLOUD_ARCHITECTURE_AWS.md in full before
starting. This document specifies how the control plane (BEACON_RELAY_BUILD_SPEC.md Section 5–7)
