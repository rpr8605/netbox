...
|---|---|---|---|
| 1 | `BEACON_RELAY_BUILD_SPEC.md` | The core spec: architecture, device agent, control plane, alerting, Fleet Console, canonical event schema, tech stack, build order (Phases 1–9), documentation standard. Read this first, always. | — |
| 2 | `BEACON_RELAY_EHR_INTEGRATIONS.md` | Which real-world EHR/EMR, interface-engine, and e-prescribing systems Beacon Relay has to recognize (17-system table), the 4-adapter integration strategy, and interface topology visibility in the Fleet Console. | Doc 1 |
| 3 | `BEACON_RELAY_CONTROLS_AND_IDENTITY.md` | Site network controls (WAN/firewall/DNS/VPN/backup/AV), the Action Registry (safe path from monitoring to limited remote action), and Microsoft 365/Entra ID read-only account-health visibility. | Doc 1 |
| 4 | `BEACON_RELAY_CLOUD_ARCHITECTURE_AWS.md` | How the control plane is actually hosted on AWS — IoT Core, RDS/Cognito, ECS Fargate, IoT Jobs, Secure Tunneling, audit log, Step Functions — service-by-service, with reasons. | Docs 1 & 3 (Action Registry, RBAC roles) |
| 5 | `BEACON_RELAY_TOPOLOGY_AND_TROUBLESHOOTING_MEMORY.md` | Geographic fleet map, deterministic (non-AI) troubleshooting memory / case-based matching, and ticketing-system export (copy-paste block + email, no vendor REST integration yet). | Docs 1 & 2 (interface topology, runbooks) |
...
This follows `BEACON_RELAY_BUILD_SPEC.md` Section 8's build order, with the companion docs
...
1. **Phase 1 — Schema & PKI foundation.** Hand Kimi `BEACON_RELAY_BUILD_SPEC.md` in full, then
   its Section 11 prompt. Nothing else matters until a device can prove who it is.
2. **Phase 2 — Minimal control plane.** Covered narratively in `BEACON_RELAY_BUILD_SPEC.md`
...
3. **Phase 3 — The Configurator.** `BEACON_RELAY_BUILD_SPEC.md` Section 12 prompt (also
   reproduced in `BEACON_RELAY_EHR_INTEGRATIONS.md` Section 1 for convenience).
4. **AWS hosting for the control plane.** Once Phase 1's step-ca/enrollment and Phase 3's
   Configurator/RAUC output exist, hand Kimi `BEACON_RELAY_CLOUD_ARCHITECTURE_AWS.md` in full
...
5. **Phases 4–5 — Device Agent + HL7/MLLP sidecar, real-world systems.** Hand Kimi
   `BEACON_RELAY_BUILD_SPEC.md` + `BEACON_RELAY_EHR_INTEGRATIONS.md` together, then the Section 10
...
6. **Interface topology visibility.** Once the Mirth/NextGen admin-API reader from step 5
   is demoable, hand Kimi `BEACON_RELAY_EHR_INTEGRATIONS.md` Section 12 prompt.
7. **Network controls + Action Registry + Identity.** Hand Kimi `BEACON_RELAY_CONTROLS_AND_IDENTITY.md`
...
8. **Fleet map + troubleshooting memory + ticketing export.** Once the per-site topology
   ...
   `BEACON_RELAY_TOPOLOGY_AND_TROUBLESHOOTING_MEMORY.md` in full plus its Section 5 prompt.
9. **Remaining Phase 6–9 items not yet split into their own companion prompts:** the
   ...
   (`BEACON_RELAY_BUILD_SPEC.md` Section 6), downtime mode and the remote-support session broker
   (`BEACON_RELAY_BUILD_SPEC.md` Section 7, Action Registry groundwork in
   `BEACON_RELAY_CONTROLS_AND_IDENTITY.md` Section 2), and the PHI-mode toggle + hardware
   lifecycle tooling (`BEACON_RELAY_BUILD_SPEC.md` Section 8, item 9). These are specified in
...
- Documentation standard from `BEACON_RELAY_BUILD_SPEC.md` Section 10 applies to every file Kimi
