...
import {
  ...
  closeSupportSession, expiredSupportSessions, appendAudit, listPendingSupportSessions,
} from '../db.js';
...
export default async function supportRoutes(app) {
  ...
  app.get('/api/support/sessions/:deviceId/pending', {
    ...
    const rows = await listPendingSupportSessions(req.params.deviceId);
  ...
  app.post('/api/support/sessions/:id/open', {
    ...
    if (!req.socket.authorized || !cn || cn !== s.device_id) {
      await appendAudit({ auditId: crypto.randomUUID(), actor: cn ?? 'unknown-device', action: 'support.open_denied', target: s.session_id, detail: 'device cert mismatch' });
      return reply.code(403).send({ error: 'device certificate does not match session device' });
    }
    if (s.state !== 'pending') {
      await appendAudit({ auditId: crypto.randomUUID(), actor: 'device', action: 'support.open_denied', target: s.session_id, detail: `state=${s.state}` });
    ...
    await openSupportSession(s.session_id);
    await appendAudit({ auditId: crypto.randomUUID(), actor: 'device', action: 'support.opened', target: s.session_id });
  ...
  app.post('/api/support/sessions/:id/close', {
    ...
    await appendAudit({ auditId: crypto.randomUUID(), actor: req.body?.actor ?? 'system', action: 'support.closed', target: s.session_id });
...
export async function sweepSupportSessions() {
  ...
  for (const s of expired) {
    await closeSupportSession(s.session_id, 'expired');
    await appendAudit({ auditId: crypto.randomUUID(), actor: 'system', action: 'support.expired', target: s.session_id });
  }
